...
# 复用原有的数据集类和其他函数
from training.train_hierarchical import HierarchicalDataset, hierarchical_collate_fn, cox_loss, train_one_epoch, evaluate

# 简单的监控函数（如果原函数不存在）
def monitor_tensor_stats(tensor, name="tensor", verbose=False):
    if verbose:
        print(f"  {name}: mean={tensor.mean():.4f}, std={tensor.std():.4f}, min={tensor.min():.4f}, max={tensor.max():.4f}")

def check_model_parameters(model, verbose=False):
    if verbose:
